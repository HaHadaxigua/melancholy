...
// HasExitlogs applies the HasEdge predicate on the "exitlogs" edge.
func HasExitlogs() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(ExitlogsTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, ExitlogsTable, ExitlogsColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasExitlogsWith applies the HasEdge predicate on the "exitlogs" edge with a given conditions (other predicates).
func HasExitlogsWith(preds ...predicate.ExitLog) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(ExitlogsInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, ExitlogsTable, ExitlogsColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
